...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (!$teamId) {
        // Buscar time do usu�rio da sess�o
        $user = getUserSession();
        ...
        if (!$row) {
            ...
            echo json_encode(['success' => false, 'error' => 'Time n�o encontrado para o usu�rio']);
            exit;
        }
        $teamId = (int)$row['id'];
    }
    ...
    try {
        $stmt = $pdo->prepare('
            SELECT id, name, foto_adicional, age, position, secondary_position, role, ovr, available_for_trade, seasons_in_league
            FROM players
            WHERE team_id = ?
